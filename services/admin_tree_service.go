...
func (adminTreeService *AdminTreeService) getMenuParent(menu map[int]models.Params, myID int, parentIds []int) []int {
	for _, a := range menu {
		if a["id"].(int) == myID && a["parent_id"].(int) != 0 {
			parentIds = append(parentIds, a["parent_id"].(int))
			parentIds = adminTreeService.getMenuParent(menu, a["parent_id"].(int), parentIds)
		}
	}
	if len(parentIds) > 0 {
		return parentIds
...
// GetLevel 递归获取级别
func (adminTreeService *AdminTreeService) GetLevel(id int, menu map[int]models.Params, i int) int {
	parentID := 0
	v, ok := menu[id]["parent_id"].(int64)
	if ok {
		parentID = int(v)
	} else {
		v1, ok := menu[id]["parent_id"].(int)
		if ok {
			parentID = v1
		}
	}
...
// initTree 初始化树
func (adminTreeService *AdminTreeService) initTree(menu map[int]models.Params) {
	adminTreeService.Array = make(map[int]models.Params)
...
func (adminTreeService *AdminTreeService) GetTree(myId int, str string, sid int, adds string, strGroup string) string {
	...
	if len(child) > 0 {
		...
		for _, id := range ids {
			...
			nStr := ""
			parentIDInt, ok := value["parent_id"].(int)
			if !ok {
				parentIDInt = int(value["parent_id"].(int64))
			}
...
func (adminTreeService *AdminTreeService) Menu(selected int, currentId int) string {
	...
	resultKey := make(map[int]models.Params)
	if result != nil {
...
func (adminTreeService *AdminTreeService) AdminMenuTree() string {
	...
	if adminMenus != nil {
		result := make(map[int]models.Params)
		var adminTreeService AdminTreeService
		for _, adminMenu := range adminMenus {
			n := adminMenu.Id
			//初始化models.Params map类型
			if result[n] == nil {
				result[n] = make(models.Params)
			}

			result[n]["id"] = adminMenu.Id
			result[n]["parent_id"] = adminMenu.ParentId
...
func (adminTreeService *AdminTreeService) AuthorizeHtml(menu map[int]models.Params, authMenus []string) string {
	for id := range menu {
